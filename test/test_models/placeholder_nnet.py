...
class NativeOpSampleModel0(TFModel):
  ...
  def _create_graph(self):
    ...
    with self.g.as_default():
      ...
      inline = self.input + x # Force matmul to not be connected to a placeholder
      ...
      self.inference = tf.nn.softmax( tf.add(tf.matmul(inline, W, name='matmul'), b, name='sum'), name='inference' )
